...
export class CanvasLimitsService {
  ...
  validateCanvasSize(
    ...
    const limits = isPremiumUser ? this.PREMIUM_USER_LIMITS : this.FREE_USER_LIMITS;
    const errors: string[] = [];
    ...
    if (rows > limits.maxRows) {
      errors.push(`Maximum number of rows is ${limits.maxRows}${isPremiumUser ? ' (Premium)' : ' (upgrade for more)'}`);
    }
    
    if (columns > limits.maxColumns) {
      errors.push(`Maximum number of columns is ${limits.maxColumns}${isPremiumUser ? ' (Premium)' : ' (upgrade for more)'}`);
    }
    
    const totalCells = rows * columns;
    if (totalCells > limits.maxTotalCells) {
      errors.push(`Total canvas size (${totalCells} cells) exceeds limit of ${limits.maxTotalCells} cells`);
    }
    
    if (totalCells > this.PERFORMANCE_LIMITS.dangerThreshold) {
      warnings.push(`Large canvas (${totalCells} cells) may cause performance issues on some devices`);
    } else if (totalCells > this.PERFORMANCE_LIMITS.warningThreshold) {
      warnings.push(`Medium canvas size (${totalCells} cells) - consider device performance`);
    }
  ...
  getLimitsForUser(isPremiumUser: boolean = false): CanvasLimits {
    return isPremiumUser ? { ...this.PREMIUM_USER_LIMITS } : { ...this.FREE_USER_LIMITS };
  }
  ...
  getRecommendedSizes(isPremiumUser: boolean = false): Array<{rows: number, columns: number, label: string}> {
    const limits = isPremiumUser ? this.PREMIUM_USER_LIMITS : this.FREE_USER_LIMITS;
